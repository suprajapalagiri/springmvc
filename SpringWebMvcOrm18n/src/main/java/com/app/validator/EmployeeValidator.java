...
public class EmployeeValidator implements Validator{
	...
	public void validate(Object target, Errors errors) {
		...
		//name accept only 4-6 chars 
		  ...
		  if(!Pattern.compile("[A-Za-z]{4,20}").matcher(e.getEmpName()).matches()) {
		...
		//pwd 2-6 upper or lower and digitis
		  ...
		  if(!Pattern.compile("[A-Za-z0-9]{2,20}").matcher(e.getEmpPwd()).matches()) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "empAddr", "empAddErr");
		  ...
		  ValidationUtils.rejectIfEmpty(errors,"empCntry", "empCntrErr");
